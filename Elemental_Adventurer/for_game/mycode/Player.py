import pygame
import os
from for_game.mycode.Father_class import Sprite
import numpy
import time
pygame.init()
...
class Player(Sprite):
    def __init__(self, start_x, start_y):
        ...
        super().__init__("for_game//image//player//Idle//1.png", start_x, start_y)
        self.all_image = {
            "run" : [pygame.image.load(f'for_game//image//player//Run//{i}.png') for i in range(1, 7)],
            "idle" : [pygame.image.load(f'for_game//image//player//Idle//{i}.png') for i in range(1, 5)]
        }
        self.anim_sleep = 0
        self.anim = 'idle'
        ...
        self.attack.image = pygame.image.load("for_game//image//obj//attack.png")
    ...
    def update(self, boxes):
        ...
        from for_game.mycode.funk import game
    ...
    def update(self, boxes):
        ...
        if lava_boxes or water_boxes:
            if self.material == "water":
                collide = pygame.sprite.spritecollide(self, lava_boxes, False)
            else:
                collide = pygame.sprite.spritecollide(self, water_boxes, False)
            if not collide:
                if key[pygame.K_q] and (current_time - self.last_material_change_time) >= self.material_cooldown:
